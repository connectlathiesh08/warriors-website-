<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Calendar.aspx.cs" Inherits="RotaractWarriors.Calendar" %>
<!DOCTYPE html>
<html lang="en">
<head>
  <script>
...
<header class="site-header sticky top-0 z-[1000] w-full shrink-0 flex items-center border-b border-transparent">
    <div class="relative flex h-full w-full items-center justify-between px-10">
      ...
      <!-- Center Navigation (Apple/Stripe Style) -->
        ...
        <a href="Calendar.aspx" data-nav="calendar" data-label="CALENDAR" class="slot-link relative px-3.5 py-1.5 text-[14px] font-bold tracking-wide transition-colors rounded-full bg-[#8B003A] text-white hover:text-white font-display"></a>
    ...
    <nav class="mobile-nav hidden border-t border-slate-100 bg-[#e0f2fe]/95 backdrop-blur-md px-6 pb-8 pt-5 min-[1440px]:hidden w-full absolute top-[70px] left-0 shadow-lg z-50">
        ...
        <li><a href="calendar.html" data-label="CALENDAR" class="slot-link block w-fit text-[17px] text-[#8B003A]"></a></li>
      </ul>
...
<main class="relative z-10 min-h-screen pt-28 pb-16 bg-white font-sans">
  <div class="max-w-[1400px] mx-auto px-4 lg:px-8">
    <div class="text-center mb-8">
      <span class="inline-block px-3 py-1 rounded-full bg-[#8B003A]/10 text-[#8B003A] text-xs font-bold uppercase tracking-wider mb-3">Interactive Schedule</span>
      <h1 class="text-3xl md:text-4xl font-black text-slate-800 tracking-tight font-display uppercase">Warriors Event Calendar</h1>
      <p class="text-slate-500 mt-2 max-w-xl mx-auto text-sm">Plan, track, and RSVP to our upcoming community services, fellowships, and club meetings in our official Google Calendar style interface.</p>
    </div>

    <!-- Google Calendar Layout Container -->
    <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 flex flex-col lg:flex-row h-auto lg:h-[750px]">
      
      <!-- Left Sidebar: Date Picker + My Calendars -->
      <aside class="w-full lg:w-[280px] bg-slate-50 border-r border-slate-100 p-6 flex flex-col gap-6 select-none shrink-0">
        

        <!-- Mini Calendar (Date Picker Grid) -->
        <div class="flex flex-col gap-2">
          <div class="flex items-center justify-between px-1">
            <span id="mini-month-year" class="text-sm font-bold text-slate-700"></span>
            <div class="flex gap-1">
              <button id="prev-mini-month" class="p-1 hover:bg-slate-200 rounded-lg text-slate-600 transition-colors">
                <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg>
              </button>
              <button id="next-mini-month" class="p-1 hover:bg-slate-200 rounded-lg text-slate-600 transition-colors">
                <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg>
              </button>
            </div>
          </div>
          <div class="grid grid-cols-7 text-center text-[10px] font-bold text-slate-400 mt-1">
            <span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span>
          </div>
          <div id="mini-calendar-grid" class="grid grid-cols-7 text-center gap-y-1 mt-1 text-xs font-semibold"></div>
        </div>
        ...
        <div class="flex flex-col gap-3">
          <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">My Calendars</span>
          <label class="flex items-center gap-3 cursor-pointer text-sm font-semibold text-slate-600 hover:text-slate-800">
            <input type="checkbox" id="filter-club" checked class="w-4 h-4 rounded text-rose-600 border-slate-300 focus:ring-rose-500" />
          ...
          <label class="flex items-center gap-3 cursor-pointer text-sm font-semibold text-slate-600 hover:text-slate-800">
            <input type="checkbox" id="filter-fellowship" checked class="w-4 h-4 rounded text-blue-600 border-slate-300 focus:ring-blue-500" />
            <span class="flex items-center gap-2">
          ...
          <label class="flex items-center gap-3 cursor-pointer text-sm font-semibold text-slate-600 hover:text-slate-800">
            <input type="checkbox" id="filter-meetings" checked class="w-4 h-4 rounded text-amber-600 border-slate-300 focus:ring-amber-500" />
            <span class="flex items-center gap-2">
      ...
      <section class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div class="h-16 border-b border-slate-100 px-6 flex items-center justify-between select-none shrink-0 bg-white">
          <div class="flex items-center gap-4">
            <button id="today-btn" class="py-1.5 px-4 border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl text-sm transition-all">
              Today
            </button>
            <div class="flex items-center gap-1">
              <button id="prev-month-btn" class="p-2 hover:bg-slate-100 rounded-full text-slate-600 transition-colors">
                <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg>
              </button>
              <button id="next-month-btn" class="p-2 hover:bg-slate-100 rounded-full text-slate-600 transition-colors">
                <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg>
              </button>
            </div>
            <h2 id="main-month-year" class="text-base md:text-lg font-bold text-slate-800 tracking-tight"></h2>
          </div>
          <!-- View selector dropdown (Statically set to Month View for optimal display) -->
          <div class="flex gap-2">
            <select id="view-select" disabled class="py-1.5 px-3 border border-slate-200 rounded-xl text-sm font-semibold text-slate-500 bg-slate-50 focus:outline-none">
        ...
        <div id="calendar-grid-container" class="flex-1 overflow-x-auto overflow-y-auto">
          <div class="min-w-[700px]">
            <div class="grid grid-cols-7 border-b border-slate-100 text-center text-xs font-bold text-slate-400 py-2.5 shrink-0 bg-slate-50">
              <span>SUN</span><span>MON</span><span>TUE</span><span>WED</span><span>THU</span><span>FRI</span><span>SAT</span>
            </div>
            <div id="main-calendar-grid" class="grid grid-cols-7 grid-rows-6 h-[560px] text-slate-700 text-xs font-semibold bg-slate-50/20"></div>
          </div>
        </div>
      </section>
    </div>
...
<div id="event-details-modal" class="fixed inset-0 z-[1100] flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300 bg-slate-900/40 backdrop-blur-sm">
  <div class="bg-white rounded-3xl shadow-2xl max-w-sm w-full mx-4 border border-slate-100 transform scale-95 transition-transform duration-300">
    <div class="p-6 flex flex-col gap-4">
      <div class="flex items-center justify-between">
        <span id="detail-category-badge" class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider"></span>
        <button class="close-modal-btn p-1.5 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
      ...
      <div class="flex flex-col gap-2">
        <h3 id="detail-title" class="text-sm font-bold text-slate-800"></h3>
        <div class="flex items-center gap-2 text-xs text-slate-500">
          <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
          <span id="detail-datetime"></span>
        </div>
        <div id="detail-location-container" class="flex items-center gap-2 text-xs text-slate-500">
          <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a8 8 0 0 0-8 8c0 5.25 8 12 8 12s8-6.75 8-12a8 8 0 0 0-8-8z"></path><circle cx="12" cy="10" r="3"></circle></svg>
          <span id="detail-location"></span>
        </div>
      </div>
      <p id="detail-desc" class="text-xs text-slate-600 leading-relaxed bg-slate-50 p-3 rounded-xl border border-slate-100"></p>
      <div class="flex gap-2.5 mt-2">
        <button id="add-to-google-btn" class="flex-1 py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-colors">
          Export to Google
        </button>
        <button id="delete-event-btn" class="py-2 px-3 border border-rose-100 hover:bg-rose-50 text-rose-600 font-bold rounded-xl text-xs transition-colors">
          Delete
        </button>
      </div>
    </div>
